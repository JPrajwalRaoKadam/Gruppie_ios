...
class SubjectNotes_videosHWTableViewCell: UITableViewCell {
    ...
    func configure(with subject: SubjectRegisterSubjectDetail) {

        SubjectLabel.text = subject.subjectName
        topics.text = "\(subject.assignedStudentsCount) topics"

        // Check if staff has profile photo
        if let staff = subject.assignedStaff.first,
           let photo = staff.profilePhoto,
           let url = URL(string: photo), !photo.isEmpty {
            ...
            // Show first letter if image not available
            iconImageView.isHidden = true
            fallbackLabel.isHidden = false
            fallbackLabel.text = subject.subjectName.prefix(1).uppercased()
        }
    }
